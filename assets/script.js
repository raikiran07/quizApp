...
function getQuestion(){
  ...
  fetch("https://opentdb.com/api.php?amount=20&category=18&difficulty=medium").then(res=>res.json()).then(data=>{
    ...
    //question
    //options - incorrect questions slice correct questions
    //correct-option - correct answer
    for(let i=0;i<20;i++){
      const items = {questionText:"",options:[],answer:""};
        let optionsList = details[i]['incorrect_answers'];
        items.answer = details[i]['correct_answer'];
        const question = details[i]['question'];
        items.questionText = question;
        optionsList.splice(Math.floor(Math.random() * (details[i]['incorrect_answers'].length + 1)), 0, details[i]['correct_answer']);
...
//calling the function to store the question before displaying it
getQuestion();
